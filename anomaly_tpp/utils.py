...
def recall_score_from_pvals(id_pvals: np.ndarray, ood_pvals: np.ndarray, alpha=0.05):
    """Compute accuracy score given p-values for ID and OOD instances."""
    y_true = np.concatenate([np.ones_like(id_pvals), np.zeros_like(ood_pvals)])
    y = np.concatenate([id_pvals, ood_pvals])
    y_pred = np.where(y > 0.05, 1, 0)
    return recall_score(y_true, y_pred)

def f1_score_from_pvals(id_pvals: np.ndarray, ood_pvals: np.ndarray, alpha=0.05):
    """Compute accuracy score given p-values for ID and OOD instances."""
    y_true = np.concatenate([np.ones_like(id_pvals), np.zeros_like(ood_pvals)])
    y = np.concatenate([id_pvals, ood_pvals])
    y_pred = np.where(y > 0.05, 1, 0)
...
def kernel_distribution_estimation(x, scores, kernel='gaussian', bw=0.3):
    n = len(scores)
    F = 1 / n * np.sum(norm.cdf((x - scores) / bw))
    return F